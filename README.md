...
- 浏览器本地加密备份与同账号恢复：可备份本地原书、译本、词汇、句子、笔记和阅读器收藏；文件只在浏览器中加密并下载，不会上传。独立备份口令无法找回，恢复采用整体替换，并拒绝跨账号恢复；
...
- 本地数据的云端同步、云端自动备份和跨账号迁移；备份的选择性恢复或自动合并；
- AI 阅读问答；
...
- 浏览器本地加密备份只读取当前账号作用域内六类固定数据，口令和备份内容不会上传或保存到云端；文件检查不写入数据，明确确认后的恢复采用整体替换，失败时尝试回滚。
