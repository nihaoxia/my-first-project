...
- 浏览器本地加密备份与按分类同账号恢复：可备份本地原书、译本、词汇、句子、笔记和阅读器收藏；检查成功后可按五个固定恢复组选择，原书与译本作为一组，选中分类整体替换，未选中分类保持原样。文件只在浏览器中加密并下载，不会上传；独立备份口令无法找回，并拒绝跨账号恢复；
...
- 本地数据的云端同步、云端自动备份和跨账号迁移；备份记录的自动合并仍未实现；
- AI 阅读问答；
...
- 浏览器本地加密备份只读取当前账号作用域内六类固定数据，口令和备份内容不会上传或保存到云端；文件检查不写入数据。恢复可全选或按五组选择，原书与译本不可拆分；只对选中分类整体替换并在失败时尝试回滚，未选中分类不读取也不写入。
