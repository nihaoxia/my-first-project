...
8. [x] 本地阶段 2 保留开发期 mock session；后续真实登录接入后，再把开发期 mock session 替换为 Supabase Auth session。
...
- 文档状态和实现状态一致。已完成。
- 所有验证命令通过。已完成。
- 本地阶段 2 数据库结构、账号系统、权限边界、账户摘要、余额流水、冻结/返还/扣费边界已完成。
- 真实 Supabase 项目、真实数据库连接、真实短信验证码和远程 Prisma 迁移仍需要用户提供外部配置后接入。
