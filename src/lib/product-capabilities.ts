...
export const homePrototypeCopy = {
  ...
  summary:
    "当前原型支持 TXT 与 EPUB 本地拆章、MCP 逐章翻译、章节阅读与学习收藏、浏览器本地语音朗读、浏览器本地加密备份与按分类同账号恢复，以及浏览器本地 TXT 与标准 EPUB 3 下载。本地语音只使用当前设备的系统本地声音，不上传正文或生成音频文件。真实翻译需要配置 MCP 与 OpenAI 兼容模型服务；EPUB 封面、图片、字体、固定布局、DRM、云端导出文件保存，以及真实计费、自动质检和生产导出管线仍未启用。",
} as const;
